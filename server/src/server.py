# -*- coding: utf-8 -*-
"""
Agora Agent & Token Service — Webhooks Recipe
...
# FastAPI application
app = FastAPI(
    title="Agora Webhooks Recipe Service",
    version="1.0.0",
    description="Agora Conversational AI — server-side NCS webhook observability",
)
...
def _generate_channel_name() -> str:
    return f"webhooks-{int(time.time())}-{random.randint(1000, 9999)}"
...
async def start_agent(request: StartAgentRequest):
    """Start events agent in a channel"""
    if agent is None:
        raise HTTPException(
            status_code=500,
            detail="Service not properly configured. Please check environment variables.",
